export const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_KEY as string;
export const STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY as string;
export const STRIPE_WH_SECRET = process.env.STRIPE_WH_SECRET as string;
export const WEBHOOK_SECRET = process.env.WEBHOOK_SECRET as string;
export const NEXT_PUBLIC_SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL as string;
export const NEXT_PUBLIC_SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY as string;
export const NEXT_PUBLIC_STRIPE_KEY = process.env.NEXT_PUBLIC_STRIPE_KEY as string;
export const NEXT_PUBLIC_SITE_URL = process.env.NEXT_PUBLIC_SITE_URL as string;
export const NEXT_PUBLIC_SITE_NAME = process.env.NEXT_PUBLIC_SITE_NAME as string;
export const TWITTER_AUTH_TOKEN = process.env.TWITTER_AUTH_TOKEN as string;